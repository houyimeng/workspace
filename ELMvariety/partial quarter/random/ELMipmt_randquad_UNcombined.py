# -*- coding: utf-8 -*-
"""
Created on Thu Jan 28 15:18:22 2016

@author: houyimeng
"""

# random manner ELM,  NOT combined version
...
for ii in range(N_sim):
    ...
    for i in range(N_train):
        ...
        random.shuffle(item_train)
        q2.train(item_train[:14*14], label_train)
        random.shuffle(item_train)
        q3.train(item_train[:14*14], label_train)
        random.shuffle(item_train)
        q4.train(item_train[:14*14], label_train)
    ...
    for i in range(N_test):
        ...
        random.shuffle(item_test)
        output_hat2 = q2.recall(item_test[:14*14])
        random.shuffle(item_test)        
        output_hat3 = q3.recall(item_test[:14*14])
        random.shuffle(item_test)        
        output_hat4 = q4.recall(item_test[:14*14])
